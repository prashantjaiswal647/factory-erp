...
# Trigger pg_dump inside Postgres container or using environment variables
if PGPASSWORD="${POSTGRES_PASSWORD:-}" pg_dump -h "${DB_HOST:-postgres}" -U "${POSTGRES_USER:-erp_admin}" -d "${POSTGRES_DB:-ai_erp}" -F p -f "${BACKUP_FILE}"; then
  ...
  find "${BACKUP_DIR}" -name "postgres_backup_*.sql" -type f -mtime +"${RETENTION_DAYS}" -exec rm -f {} \; -print | tee -a "${LOG_FILE}"
