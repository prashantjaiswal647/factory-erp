...
export default function BriefingCard() {
    ...
    return (
      <section className="rounded-xl border border-brand-200 bg-brand-50 p-4 shadow-sm" aria-label="Morning briefing">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-sm font-bold text-zinc-950">Morning Briefing</h2>
            <p className="mt-3 text-sm text-zinc-500">Loading briefing...</p>
          </div>
        </div>
      </section>
    );
  ...
  const missingData = safeArray(briefing.missing_data);
  const aiExplanation = briefing.ai_explanation;
  const aiObservability = briefing.ai_observability;
      ...
      <section className="rounded-xl border border-brand-200 bg-brand-50 p-4 shadow-sm" aria-label="Morning briefing">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <pre className="mt-3 whitespace-pre-wrap font-sans text-sm leading-6 text-zinc-800">{briefing.message_text || "No briefing text available."}</pre>
            {missingData.length > 0 ? (
              <div className="mt-3 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                ...
                <span>Missing data: {missingData.join(", ")}</span>
              </div>
            ) : null}
          </div>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-4 space-y-3">
                {aiExplanation.cost_explanation && (
                  <div>
                    <h3 className="text-xs font-bold text-indigo-950">Cost Analysis</h3>
                    <p className="text-xs text-zinc-700 mt-0.5 leading-relaxed">{aiExplanation.cost_explanation}</p>
                  </div>
                )}
                
                {aiExplanation.health_explanation && (
                  <div>
                    <h3 className="text-xs font-bold text-indigo-950">Factory Health</h3>
                    <p className="text-xs text-zinc-700 mt-0.5 leading-relaxed">{aiExplanation.health_explanation}</p>
                  </div>
                )}
                
                {aiExplanation.wastage_explanation && (
                  <div>
                    <h3 className="text-xs font-bold text-indigo-950">Wastage Audit</h3>
                    <p className="text-xs text-zinc-700 mt-0.5 leading-relaxed">{aiExplanation.wastage_explanation}</p>
                  </div>
                )}
                
                {aiExplanation.profit_explanation && (
                  <div>
                    <h3 className="text-xs font-bold text-indigo-950">Profitability</h3>
                    <p className="text-xs text-zinc-700 mt-0.5 leading-relaxed">{aiExplanation.profit_explanation}</p>
                  </div>
                )}
                
                {aiExplanation.per_size_explanation && (
                  <div>
                    <h3 className="text-xs font-bold text-indigo-950">Size Performance</h3>
                    <p className="text-xs text-zinc-700 mt-0.5 leading-relaxed">{aiExplanation.per_size_explanation}</p>
                  </div>
                )}
              </div>
              
              {aiExplanation.action_items && safeArray(aiExplanation.action_items).length > 0 && (
                <div className="mt-4 pt-4 border-t border-indigo-100/50">
                  ...
                  <ul className="mt-2 space-y-1.5">
                    {safeArray(aiExplanation.action_items).slice(0, 3).map((item, idx) => (
                ...
                <div className="mt-4 pt-3 border-t border-indigo-100/30">
                  ...
                  {showObs && (
                    <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-2 rounded-lg bg-zinc-900/5 p-3 text-[10px] font-medium text-zinc-600">
                      <div><span className="font-semibold">Model Name:</span> {aiObservability.model_name || "N/A"}</div>
                      <div><span className="font-semibold">Cache Status:</span> {aiObservability.cache_hit ? "HIT (No LLM Cost)" : "MISS"}</div>
                      <div><span className="font-semibold">Generation Time:</span> {aiObservability.generation_time}s</div>
                      <div><span className="font-semibold">Token Usage:</span> {aiObservability.token_usage}</div>
                      {aiObservability.fallback_reason && (
                        <div className="col-span-2 text-red-600"><span className="font-semibold">Fallback Reason:</span> {aiObservability.fallback_reason}</div>
                      )}
                    </div>
                  )}
                </div>
