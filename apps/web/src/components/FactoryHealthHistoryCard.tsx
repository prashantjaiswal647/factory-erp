...
function HealthLineChart({ items, onSelect }: { items: FactoryHealthHistoryItem[]; onSelect: (item: FactoryHealthHistoryItem) => void }) {
  ...
  const points = useMemo(() => list.map((item, index) => {
    const x = list.length === 1 ? width / 2 : (index / (list.length - 1)) * width;
    const y = height - ((item.overall_score ?? 0) / 100) * height;
    return { item, x, y };
  }), [list]);

  if (!list.length) return <p className="py-8 text-center text-sm text-zinc-500">No health history available.</p>;
...
function DayPanel({ item }: { item: FactoryHealthHistoryItem }) {
  const components = [
    ["Production", item.production_score ?? 0],
    ["Attendance", item.attendance_score ?? 0],
    ["Collections", item.collections_score ?? 0],
    ["Inventory", item.inventory_score ?? 0],
    ["Cost", item.cost_score ?? 0],
  ] as const;
  return (
...
export default function FactoryHealthHistoryCard() {
  ...
  useEffect(() => {
    void getFactoryHealthHistory(30).then((result) => {
      ...
      setSelected(items.length ? items[items.length - 1] : null);
    }).catch(() => setData(null));
  }, []);
  ...
  return (
    <section className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm" aria-label="Factory health history">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className={`rounded-full px-3 py-1 text-xs font-bold ${trendClasses[trendDir] || trendClasses.STABLE}`}>{trendDir}</span>
      </div>
      ...
      <HealthLineChart items={items} onSelect={setSelected} />
