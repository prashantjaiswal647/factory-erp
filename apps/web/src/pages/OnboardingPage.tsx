...
export default function OnboardingPage() {
  ...
  async function saveSignature(role: AuthorizedSignatureRole, file: File) {
    try {
      await uploadAuthorizedSignature(role, file);
      await loadSignatures();
      setToast("Authorized signature saved");
    } catch (error) {
      const detail = (error as { response?: { data?: { detail?: string } } }).response?.data?.detail;
      setToast(detail || "Signature upload failed");
    }
  }

  async function removeSignature(role: AuthorizedSignatureRole) {
    await deleteAuthorizedSignature(role);
    await loadSignatures();
    setToast("Authorized signature removed");
  }
        ...
        <Panel icon={Factory} title="Company Profile Details">
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4">
              <h3 className="font-semibold text-zinc-950">Authorized Signatures</h3>
              <p className="mt-1 text-xs text-zinc-500">This signature will appear automatically on invoices generated by this role.</p>
              {signaturesLoading ? (
                <div className="mt-4 rounded-lg border border-zinc-200 bg-white p-4 text-sm text-zinc-500" role="status">
                  Loading authorized signatures...
                </div>
              ) : null}
              {signaturesError ? (
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-3">
                {([
                  ...
                  ["sub_owner", "Sub-owner Signature"],
                  ["supervisor", "Supervisor Signature"],
                ] as const).map(([role, label]) => {
                  const signature = (Array.isArray(signatures) ? signatures : []).find((item) => item.role === role);
                  const normalizedRole = (user?.role || "").toLowerCase().replace("-", "_");
                  const canManage = normalizedRole === "owner" || normalizedRole === role;
                  return (
                    <div key={role} className="rounded-lg border border-zinc-200 bg-white p-3">
                      <p className="text-sm font-semibold text-zinc-900">{label}</p>
                      {signature ? <img className="mt-3 h-16 w-full object-contain" src={signature.url} alt={label} /> : <div className="mt-3 grid h-16 place-items-center rounded bg-zinc-50 text-xs text-zinc-400">Not uploaded</div>}
                      {canManage ? (
                        <div className="mt-3 flex items-center gap-2">
                          <label className="cursor-pointer rounded-md bg-brand-600 px-3 py-2 text-xs font-semibold text-white">
                            {signature ? "Replace" : "Upload"}
                            <input className="hidden" type="file" accept=".png,.jpg,.jpeg,.webp,image/png,image/jpeg,image/webp" onChange={(event) => {
                              const file = event.target.files?.[0];
                              if (file) void saveSignature(role, file);
                              event.target.value = "";
                            }} />
                          </label>
                          {signature ? <button className="text-xs font-semibold text-red-600" type="button" onClick={() => void removeSignature(role)}>Delete</button> : null}
                        </div>
                      ) : null}
                    </div>
                  );
